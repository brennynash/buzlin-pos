...
class LocalStorage {
  ...
  static bool getLangLtr() => !(_preferences?.getBool(StorageKeys.keyLangLtr) ?? true);
  ...
  static String getPinCode() => _preferences?.getString(StorageKeys.pinCode) ?? '';
  ...
  static Future<void> setAddress(LocationData locationData) async {
    if (_preferences != null) {
      ...
      await _preferences!
          .setString(StorageKeys.keyAddress, location);
    }
  }

  static LocationData? getAddress() {
    final savedString =
    _preferences?.getString(StorageKeys.keyAddress);
    if (savedString == null) {
      return null;
    }
    final map = jsonDecode(savedString);
