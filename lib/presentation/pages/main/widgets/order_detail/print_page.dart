...
import 'package:admin_desktop/infrastructure/printer/printer_help.dart';
...
class _PrintPageState extends State<PrintPage> {
  var defaultPrinterType = Platform.isAndroid || Platform.isIOS
  ...
  Future _printReceiveTest() async {
    ...
    bytes += generator.row([
      ...
      PosColumn(
        width: 6,
        text: TimeService.dateFormatMDYHm(widget.orderData?.createdAt?.toLocal() ?? DateTime.now()),
        styles: const PosStyles(align: PosAlign.right),
      ),
    ]);
    ...
    for (int index = 0;
      ...
      bytes += generator.row([
        PosColumn(
          width: 8,
          text:
              "$customSpace${widget.orderData?.details?[index].product?.translation?.title ?? ""} x ${widget.orderData?.details?[index].quantity ?? ""}",
    ...
    bytes += generator.row([
      ...
      PosColumn(
        width: 6,
        text: AppHelpers.numberFormat(
            number: widget.orderData?.details?.first.discount),
        styles: const PosStyles(align: PosAlign.right, bold: true),
      ),
    ]);
    bytes += generator.row([
